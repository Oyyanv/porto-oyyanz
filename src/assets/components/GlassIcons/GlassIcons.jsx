const gradientMapping = {
  ...
  white: "linear-gradient(#959c9c, #232424)",
};
...
const GlassIcons = ({ items, className }) => {
  ...
  return (
    <div
      className={`grid gap-[3em] grid-cols-3 md:grid-cols-3 mx-auto py-[4em] overflow-visible ${className || ""
        }`}
    >
      {items.map((item, index) => (
        ...
        >
          ...
          <span className="absolute top-full left-0 right-0 text-center whitespace-nowrap leading-[2] text-base opacity-100 transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.192,0,0.17,1)] translate-y-0 group-hover:opacity-100 group-hover:[transform:translateY(5%)]">
            {item.label}
          </span>
        </button>
      ))}
